...
export async function GET(request: Request) {
  ...
  if ("fallbackRun" in result && result.fallbackRun) {
    return new Response(renderFallbackPreview(result), {
      status: 200,
      headers: {
...
function renderFallbackPreview(result: Awaited<ReturnType<typeof handleApprovalToken>>) {
  const run = "fallbackRun" in result ? result.fallbackRun : null;
  ...
  const shareUrl = `https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(run.journalUrl)}`;
...
<body>
  <main>
    <section class="panel">
      <div class="label">Secure LinkedIn fallback</div>
      <h1>${escapeHtml(run.journalTitle)}</h1>
      <p>${escapeHtml(result.message)}</p>
      <div class="note">${escapeHtml("fallbackReason" in result ? result.fallbackReason || "" : "")}</div>
      <p><strong>Journal:</strong> <a href="${escapeAttribute(run.journalUrl)}" target="_blank" rel="noopener noreferrer">${escapeHtml(run.journalUrl)}</a></p>
      <label class="label" for="linkedin-post">Final LinkedIn post</label>
      <textarea id="linkedin-post" spellcheck="true">${escapeHtml(run.linkedinDraft)}</textarea>
      <div class="actions">
        <button class="primary" type="button" id="copy-button">Copy post</button>
        <a class="button primary" href="${linkedInComposerUrl}" target="_blank" rel="noopener noreferrer">Open LinkedIn composer</a>
        <a class="button" href="${shareUrl}" target="_blank" rel="noopener noreferrer">Open LinkedIn share link</a>
      </div>
      <p class="small">Copy the full post first, then open LinkedIn and paste. LinkedIn does not reliably allow prefilled long-form post text from external websites. This page never exposes LinkedIn tokens and does not post automatically.</p>
